...
export default function Contact() {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        setState({ ok: true, message: "Message sent — I'll get back to you soon." });
  ...
  return (
    <footer className="contact wrap" id="contact">
      ...
      <h2 className="contact-big reveal">
        Let&apos;s build
        <br />
        something <span style={{ color: "var(--accent)" }}>that thinks.</span>
      </h2>
      ...
      <div className="contact-grid">
        <form ref={formRef} className="cf-form reveal" onSubmit={submit}>
          ...
          {state.message && (
            <div className={state.ok ? "cf-sent" : "cf-error"}>{state.message}</div>
          )}
          <button className="cf-submit" type="submit" disabled={isPending}>
            {isPending ? "Sending…" : <>Send message <span className="circle">{Ic.send()}</span></>}
          </button>
        </form>

        <div className="cf-side reveal">
          <h3>Prefer something direct?</h3>
          <p>Drop a line and I&apos;ll get back within a day. I&apos;m open to senior full-stack and AI-engineering roles.</p>
      ...
      <div className="foot-bottom">
        <span>© 2026 Abhishek Mahato — engineered with clean code &amp; coffee ☕</span>
